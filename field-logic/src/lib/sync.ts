...
const uploadAudioToDrive = async (
  ...
  console.log(
    `[Stub] Uploading ${filename} to Google Drive with token ${token}...`,
  );
...
export const flushAudioBlobs = async (token: string = "test_token") => {
  ...
  const results = await Promise.all(
    unsyncedBlobs.map(async (record) => {
      ...
      try {
        ...
        return { sessionId, nodeId, status: "synced", url };
      } catch (e) {
        console.error(`Failed to sync ${filename}`, e);
        return { sessionId, nodeId, status: "failed", error: e };
      }
    }),
  );
